...
import { ChatSession, ChatMessage as ChatMessageType, ApiError } from '@/lib/types';
...
export default function ChatPage() {
  ...
  const handleSendMessage = async (content: string) => {
    ...
    try {
      ...
      const errorMsg = errorDetail === 'Service is unavailable. Please check your connection.'
        ...
        <div className="flex items-center justify-between p-4 border-b border-[var(--md-sys-color-outline)] bg-[var(--md-sys-color-surface)]">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-semibold text-[var(--md-sys-color-on-surface)]">
              {(currentSession?.title && currentSession.title !== '' && currentSession.title !== 'New Chat' && currentSession.title !== 'Новый чат') ? currentSession.title : t('sidebar.newChat')}
            </h1>
          </div>
