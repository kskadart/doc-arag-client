...
export function TaskProgress({ taskId, onComplete, onError }: TaskProgressProps) {
  ...
  const progress =
    status.total_chunks > 0 ? (status.chunks_processed / status.total_chunks) * 100 : 0;
    ...
    <div className="mt-6 p-6 bg-[var(--md-sys-color-surface)] border border-[var(--md-sys-color-outline)] rounded-2xl">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          {status.status === 'completed' && <CheckCircle className="w-6 h-6 text-green-600" />}
      ...
      {status.status === 'processing' && (
        <div>
          ...
          <p className="text-sm text-[var(--md-sys-color-on-surface-variant)] text-center">
            {t('chunks', { current: status.chunks_processed, total: status.total_chunks })} (
            {Math.round(progress)}%)
          </p>
        </div>
      )}
      ...
      {status.status === 'failed' && (
        <div className="text-center text-sm text-red-600 dark:text-red-400">{status.message}</div>
      )}
    </div>
  );
}
